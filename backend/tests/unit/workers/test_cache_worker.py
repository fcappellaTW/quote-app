...
from app.worker import populate_cache_if_needed
...
def test_populate_cache_calls_ai_service_when_quote_count_is_low(mocker):
   ...
   """
   mock_redis_client = mocker.patch("app.worker.redis_client")
   mock_redis_client.llen.return_value = 2
   MINIMUM_QUOTE_COUNT = 5

   mock_ai_quote_service = mocker.patch("app.worker.generate_ai_quotes")
